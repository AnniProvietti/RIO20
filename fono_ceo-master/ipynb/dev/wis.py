...
class WisconsinPlayers:
    ...
    PARAMSTRIAL = "categoria xpos cor score acertos house outros numero ypos player state result time marker".split()
    PARAMSHOUSES = "h b m l categoria acertosConsecutivos indiceCartaAtual outrosConsecutivos wteste".split()
    PARAMSDADOS = "ano1 idade1 sexo1 starttime endtime tipoescola escola name maxlevel timeOne houses categoria xpos cor score acertos house outros numero ypos player state result timeTrial marker criteria markers trial headings timeGoal level".split()
    ...
    def file_demographics(self, date="2012", start_count=(0, 2000), name="wisconsin.csv"):
        ...
        tr = [goals["trial"] for y in reg for game in self.one_player(y)["games"] for goals in game["goal"] if game["name"] == "wisconsin"]
        ...
        print(len(df_dados))
        print(len(df_one))
        # print(len(df_one_goal))
        print(len(df_one_houses))
        print(len(tr))
        print(tr)
        print(len(df_one_trial))
        print(len(df_one_criteria))
        print(len(df_one_markers))
        print(len(df_one_headings))
        print(len(df_one_time))
        print(len(df_one_level))
